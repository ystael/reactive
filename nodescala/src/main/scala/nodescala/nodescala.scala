...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val handlerSubscription = Future.run()(token => Future {
      def loop(awaiter: Future[(Request, Exchange)]): Unit = {
        while (token.nonCancelled) {
          if (awaiter.isCompleted) {
            for { (request, exchange) <- awaiter } yield respond(exchange, token, handler(request))
            loop(listener.nextRequest())
          }
        }
      }
      loop(listener.nextRequest())
